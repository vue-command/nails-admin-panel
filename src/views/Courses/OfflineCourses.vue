<template>
   <v-card flat dark>
     ...
     <v-btn @click="openForm('open')" v-if="showAddBtn">add new offline course</v-btn>
     <v-btn @click="openForm('')" v-if="showBackBtn">back</v-btn>
...
export default {
  ...
  methods: {
    ...
    async getMoreOfflineCourses () {
      const response = await(await fetch(`https://nails-australia-staging.herokuapp.com/course/offline?skip=${this.offlineCourses.length}`)).json()
    ...
    openForm (type) {
      this.showForm = !!type
      this.showCourses = !type
      this.showAddBtn = !type
