...
export default {
  ...
  methods: {
    ...
    async publishHandler() {
      try {
        let commodityToUpdate = this.currentProduct;
        delete commodityToUpdate.images;
        console.log('commodityToUpdate', commodityToUpdate);
        let updatedCommodity = await this.updateCommodity(this.productId, {
          ...commodityToUpdate,
    ...
    async submitHandler() {
      ...
      if (!this.isEditMode) {
        ...
        let createdCommodity = await this.createCommodity(data);
        if (createdCommodity) {
        ...
        let updatedCommodity = await this.updateCommodity(this.productId, data);
        if (updatedCommodity) {
          this.currentProduct = updatedCommodity;
          this.getDataMain();
        }
